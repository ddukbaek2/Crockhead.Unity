	...
	public static class UIHelper
	{
		...
		public static IUIView CreateView(string path, AssetPathType type)
		{
			try
			{
				...
				if (view == null)
					throw new MissingComponentException($"[Crockhead.Unity.UIKitLite.Deprecated] CreateView(): Path=`{path}`, Type=`{type}`");
				return view;
			}
		...
		public static IUIView CreateView(Type viewControllerType)
		{
			try
			{
				...
				if (!Reflections.IsBaseClass(viewControllerType, typeof(UIViewController)))
					throw new InvalidCastException($"[Crockhead.Unity.UIKitLite.Deprecated] CreateView(): {viewControllerType}");

				// AssetPathAttribute 특성 체크.
				if (!Reflections.TryGetAttribute<AssetPathAttribute>(viewControllerType, out var assetPathAttribute))
					throw new InvalidOperationException("[Crockhead.Unity.UIKitLite.Deprecated] CreateView(): Not found AssetPathAttribute.");
