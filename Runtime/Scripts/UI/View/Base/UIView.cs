...
namespace Crockhead.Unity.UI
{
	...
	public class UIView : UINode, IUIView, IUIConstraintable
	{
		...
		/// 지정 버튼 클릭 이벤트 연결.
		/// </summary>
		public UIButtonView BindButtonClickEvent(string transformPath, UnityAction action)
		{
			var button = GetOrAddComponent<UIButtonView>(transformPath);
			if (button == null)
			{
				Debug.Log($"[{ComponentName}] BindButtonClickEvent({transformPath})");
				return null;
			}
		...
		/// 지정 버튼 클릭 이벤트 제거.
		/// </summary>
		public UIButtonView UnbindButtonClickEvent(string transformPath, UnityAction action)
		{
			var button = GetOrAddComponent<UIButtonView>(transformPath);
			if (button == null)
			{
				Debug.Log($"[{ComponentName}] UnbindButtonClickEvent({transformPath})");
				return null;
			}
		...
		public void UnbindAllButtonClickEvents(string transformPath)
		{
			...
			if (button == null)
			{
				Debug.Log($"[{ComponentName}] UnbindAllButtonClickEvents({transformPath})");
				return;
			}
